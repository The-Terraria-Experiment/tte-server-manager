...
export const runCommand = async (event: AuthorizedEvent) => {
	...
	const { command } = event.parsedBody || {};

	if (!command || typeof command !== "string" || !command.trim()) {
		return ResponseUtil.ValidationError("Command is required");
	}
	...
	try {
		...
		const result = await tshock.APIRequest(userId!, "/v3/server/rawcmd", { cmd: command });

		await CWLogger.Action(FUNC_NAMES.SERV_MGR, {
			userId,
			action: "run-command",
			resource: `${event.httpMethod ?? "unknown method"}: ${event.path ?? "unknown path"}`,
			details: { serverId, command, result },
		});

		return ResponseUtil.Success({ response: result?.response ?? result });
	} catch (error: any) {
		return ResponseUtil.Error(error?.message || "Failed to run command");
	}
};
