...
- **Lambda Functions** (`src/lambda/*`, one dir per function):
  ...
  - `auto-shutoff-manager`: idle-EC2 detection/shutdown (queue-driven, `AUTO_SHUTOFF_*` env vars).
...
- **CI/CD:** `.github/` contains the current CI workflows — check there rather than assuming a pipeline; Lambda deploys are handled outside this repo (no CDK/Terraform checked in here).
