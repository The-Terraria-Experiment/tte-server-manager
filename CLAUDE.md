...
- **Graphs:** `common/Graph.vue` takes points with two independent optional keys. `group` is a *drawing* concern — each group is stroked as one continuous path, so a caller starts a new group wherever the line should break (a gap in the data). `series` is a *styling* concern — the key looked up in the `seriesConfig` prop for color/line weight/point weight/hover label, falling back to `group` when absent. One series normally spans many groups: two lines that each break over the same three gaps are six groups but two colors. A second series must therefore be given group numbers offset past the first's, or the two get stroked as one path.
...
- **Endpoints (representative — grep `src/tte-server-manager/src` for `get(`/`post(` for the current full set):**
  ...
  - `GET /instance/{id}/metrics?start=&end=&maxPoints=` → the collected CPU/memory samples for a window (epoch seconds; defaults to the last 6h, capped at 14 days), read out of S3 and bucketed down to `maxPoints`
...
- Reading the metric *data* (`GET /instance/{id}/metrics`, `_shared/shared/utils/instance/InstanceMetricsData.ts`) is a pure S3 read — it never touches the box, so it costs the instance nothing and works identically against a stopped one. Consequences: (1) the newest point is as old as the last upload, not as old as the last sample, which is why the response carries `lastSampleAt` and the UI labels it; (2) `instance-manager`'s role needs **`s3:GetObject` on `{logs bucket}/metrics/*`**, which it did not previously have — every other use of `S3_LOGS_BUCKET_NAME` in that function only builds SSM commands for the *instance's* role to run, so the lambda itself had no read on that bucket; (3) which hour objects to fetch is *computed* from the requested window rather than listed, so the key layout must be UTC everywhere — `tte-metrics-collect.sh` pins `TZ=UTC` before formatting the path for exactly this reason. Samples are filtered on their own `t` field regardless, so a stray local-time path costs only the shifted hours at the window edges.
- The metrics read fetches one S3 object per hour in the window (14 days = 336 objects) and is behind **API Gateway's 29s integration timeout** like every other metrics action, so it stops at `METRICS_READ_BUDGET_MS` (18s) and returns what it has with `truncated: true` + `coveredStart`. Hours are enumerated **newest-first** precisely so a truncated read keeps recent history rather than the oldest. Raw samples are bucketed to at most `maxPoints` (default 750) — a 15s cadence over 14 days is ~80k samples, which is megabytes of JSON to draw the same picture on a canvas a thousand pixels wide. Empty buckets are omitted rather than zero-filled (a stopped instance produced no data; plotting 0% CPU for it would be a false claim about the box), and the resulting gaps drive the `group` field that makes `Graph.vue` break the line instead of drawing a straight edge across the downtime. Each bucket carries `cpuMax` alongside the average and the UI plots both as separate series, because averaging is exactly what hides a spike — a minute at 100% inside a 30-minute bucket reads as 3%. Memory gets no peak series: it's a level rather than a rate, so the two lines would sit on top of each other.
- The metrics *config* read endpoint returns `configured` (whether `metricsConfig` exists) because `toMetricsConfig` *fabricates* defaults when it doesn't — without the flag, a box that never ran the collector reads back as a confident "Enabled, 60s sampling". The default read is Dynamo-only, so there is no `actual` to contradict it either. The tile shows "Not configured" and labels the values as defaults; only `?verify=true` can say "Not installed".
...
- The instance list is the **`inst#<id>` rows themselves** — an `envs` array (`["prod"]`, `["stage"]`, or both) on the row is the registration, read through `_shared/shared/utils/instance/InstanceRegistry.ts`. This replaced the `EC2_INSTANCE_IDS` and `AUTO_SHUTOFF_SERVER_IDS_PROD`/`_STAGE` env vars, which were three hand-synced copies of one list (`logs-manager` carried a third that nothing read) that could only reach an alias via a redeploy. Points worth knowing before changing it:
  ...
  - **A row existing is not a registration.** `setup.sh`'s `register` step seeds `validRoots`/`worldPaths`/`metricsConfig` on a fresh box without `envs`, so a provisioned-but-unregistered instance is a normal state. **Both** writers use `UpdateItem`, never `PutItem`, and for symmetric reasons: registering can't clobber the paths setup.sh wrote, and setup.sh's `register` step can't clobber the `envs`/`name`/`registeredAt` the UI wrote (a `PutItem` there would silently *deregister* an instance already added in the UI). That step also needs `dynamodb:GetItem` + `dynamodb:UpdateItem` on the instance role, or it dies — see `instance-scripts/setup/README.md`, "Instance role".
