...
- **Lambda env vars (vary by function — check the function's own code before assuming one applies elsewhere):**
  ...
  - `AUTO_SHUTOFF_IDLE_MINUTES`, `AUTO_SHUTOFF_EC2_DELAY_MINUTES`, `AUTO_SHUTOFF_SERVER_IDS_PROD`/`_STAGE` (falls back to `AUTO_SHUTOFF_SERVER_IDS`), `AUTO_SHUTOFF_SCHEDULER_ROLE_ARN` (role EventBridge Scheduler assumes to invoke this lambda), `INSTANCE_MANAGER_FUNCTION_ARN_PROD`/`_STAGE` (alias-qualified ARN of `instance-manager`, which auto-shutoff invokes to run the shutdown job — its role also needs `lambda:InvokeFunction` on that function)
