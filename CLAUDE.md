...
- **Lambda Functions** (`src/lambda/*`, one dir per function):
  ...
  - `tshock-proxy`: VPC-attached transport for TShock REST calls. Invoked synchronously by `server-manager` and `auto-shutoff-manager`; makes the HTTP request from inside the VPC so the instances' REST port can stay closed to the internet. Makes **no AWS API calls at all** — see the gotcha below before adding one.
